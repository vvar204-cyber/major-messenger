...
# ===== Абсолютные пути относительно расположения main.py =====
...
def send_friend_request(
    ...
    target_user = db.query(models.User).filter(models.User.username == request.to_username).first()
    if not target_user:
...
async def websocket_endpoint(websocket: WebSocket, user_id: int, db: Session = Depends(get_db)):
    ...
    try:
        while True:
            ...
            # ===== Групповые звонки: присоединение =====
            ...
            # ===== Групповые звонки: ICE candidates =====
